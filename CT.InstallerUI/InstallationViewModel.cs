...
namespace CT.InstallerUI
{
    ...
    public class InstallationViewModel : PropertyNotifyBase
    {
            ...
            get
            {
                if (this.installCommand == null)
                {
                    this.installCommand = new RelayCommand(
                        ...
                        param => this.root.DetectState == DetectionState.Absent && this.root.UpgradeDetectState == UpgradeDetectionState.None && this.root.InstallState == InstallationState.Waiting);
                }
        ...
        {
            WixBA.Model.Engine.SetVariableString("Install_Dir", Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), false);
            WixBA.Model.Engine.SetVariableNumeric("INSTALL_CTCLIENT_PACKAGE", 0);
